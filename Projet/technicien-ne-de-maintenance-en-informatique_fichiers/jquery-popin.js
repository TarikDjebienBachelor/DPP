jQuery.fn.popin = function(o) {
	...
	// Modifi� par : Abdelkader  RHOUATI	
	// Date : 24/12/2009
	// D�scription : Ajouter deux parametre; method et formData
	...
	jQuery(this).each(function() {
		jQuery(this).click(function() {
			...
			// Modifi� par : Abdelkader  RHOUATI	
			// Date : 12/01/2009
			// D�scription : possibilit� d'utilisation de l'attribue rel si l'attribue href n'existe pas 
			// 				( c'est � dire on n'est pas sur un lien )
			//MAJ par makasmi this.hasAttribute => $(this).attr("href") pour IE7
			if($(this).attr("href")){
				PPNopen($(this).attr("href"));
				return false;
			}else if($(this).attr("rel")){
				PPNopen($(this).attr("rel"));
				return true;
			}
	...
	function PPNopen(url) {
		...
		$(".popin-voile").animate({opacity:settings.opacity, height:((ie6 == true) ? (PPNbodyHeight + PPNbodyMargin.top + PPNbodyMargin.bottom) : "100%")}, function() {
			...
			// Modifi� par : Abdelkader  RHOUATI	
			// Date : 24/12/2009
			// D�scription : dynamisaer la methode de la requette ajax GET/POST
			//				 dynamiser variable data (donn�e envoy� avec la requette)

			// Requ�te
			$.ajax({
				type: settings.method,
				url: url,
				data: $('#'+settings.formData).serialize(),
				dataType: "html",
				success: function(m){
				...
				complete: function(){
					...
					// Action fermeture
					$(".popin-close, .popin-voile, .popin-visu-organisme").click(function() {
						PPNclose();
						return false;
					});
				}
